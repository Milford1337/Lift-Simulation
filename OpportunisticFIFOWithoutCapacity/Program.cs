// --- The program simulates a lift that can accommodate multiple passengers at once, but which has no capacity.
...
// --- (iii) takes 5 seconds to collect a passenger; and (iv) takes 5 seconds to drop a passenger.
// --- Each of (i)-(iv) can be adjusted by passing custom int values into the Lift constructor when the Lift object is created in Main().
...

{
    ...
    public class Lift
    {
        ...
        public Lift(int currentfloor = 1, int timebetweenfloors = 10, int collectiontime = 5, int droptime = 5)
        {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (servedPassengers.Count < passengers.Count && currentTime < 10000)        // Added the second conjunct for safety!
            {
                ...
                // The lift now checks to see whether that are extra Passengers to collect at the CurrentFloor.
                // There are two sub-cases:

                // (2.A.ii.a) There are extra Passengers to collect at the CurrentFloor.
                ...
                // (2.A.ii.b) There are no extra Passengers to collect at the CurrentFloor.
                    ...
                    else                                // (2)
                    {
                        ...
                        if (lift.CurrentFloor != lift.TargetFloor)              // (2.A)
                        {
                            ...
                            else                                                // (2.A.ii)
                            {
                                ...
                                if (extrasToCollect.Count > 0)                  // (2.A.ii.a)
                                {
                            ...
                            if (lift.PassengersInLift.Contains(lift.TargetPassenger))       // (2.B.i)
                            {
                                ...
                                else                                                        // (2.B.i.b)
                                {
                                    ...
                                    else                                                    // (2.B.i.b.ii)
                                    {
                                        ...
                                        if (extrasToCollect.Count > 0)                      // (2.B.i.b.ii.A)
                                        {
                                            lift.CollectingPassenger = (from p in extrasToCollect
                                                                        orderby p.StartTime, p.Id
                                                                        select p).First();
                ...
                // So, we check for these extra Passengers.
                // There are two cases:

                // (2.A) There are extra Passengers to collect at CurrentFloor.
                // The lift moves into the CollectingExtra state.
                // We select one of the extra external passengers.
                // The CollectingPassenger is updated to this selected passenger.
                // We set TimeToCollectionEnd to CollectionTime.
                ...
                // (2.B) There are no extra Passengers to collect at CurrentFloor.
                ...
                if (lift.State == LiftState.Collecting)
                {
                    ...
                    else                                                                    // (2)                                              
                    {
                        ...
                        var extrasToCollect = (from p in lift.PassengerCalls
                                               where p.StartFloor == lift.CurrentFloor && !lift.PassengersInLift.Contains(p)
                                               select p).ToList();
                        if (extrasToCollect.Count > 0)                                      // (2.A)
                        {
                            lift.CollectingPassenger = (from p in extrasToCollect
                                                        orderby p.StartTime, p.Id
                                                        select p).First();
                ...
                // We first check if there are extra external passengers to collect.
                // Two cases.

                // (2.A)
                // There are extra external passengers to collect.
                // We select the next such passenger to collect.
                // We update CollectingPassenger with the selected Passenger.
                // The lift moves into the CollectingExtra state.
                // We set TimeToCollectionEnd to CollectionTime.
                ...
                // (2.B)
                // There are no extra external passengers to collect.
                // We check whether the TargetPassenger is in PassengersInLift.
                // If it is, the lift moves to the MovingToDrop state.
                // If it is not, the lift moves to the MovingToCollect state.
                // Either way, we set TimeToNextFloor to TimeBetweenFloors.
